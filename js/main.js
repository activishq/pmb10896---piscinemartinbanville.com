...
function testimonialsCarousel(){
	...
	$('.testimonial_carousel').each(function(){
		...
		$(this).carouFredSel({
			...
			auto: true,
			scroll: { items : 1, fx: 'fade', duration:4000 },//add here the duration of a testimon. 
			prev : {
...
function codelessPortfolioPageIsotope(){
	...
	if($('#portfolio-preview-items .filterable').length > 0){
		...
		var filter_show = null; 
		if($("#set-default")){
			filter_show = $("#set-default").attr("data-filter"); //GET FIRST PAGE TO SHOW
		}	
...
/* ----------------------- Dynamic ContactForm GTM ----------------------- */ 
$(document).ready(function () {

	$('.wpcf7').on('mailsent.wpcf7', function(event) {
		var form_id = $(event.currentTarget).find('input[name="_wpcf7"]').val();
		if( form_id == '1225' ){
			dataLayer.push({
				'event': 'gaEvent',
				'eventCategory' : 'Form',
		...
		else if( form_id == '1319' ){
			dataLayer.push({
				'event': 'gaEvent',
				'eventCategory' : 'Form',
				'eventAction': 'Submit',
				'eventLabel': 'Form_submission'
			});
			console.log('Sending unicorns along your way, Google.');
		}
	});
